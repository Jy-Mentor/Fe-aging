...
v28 更新:
  - CPI v29 补充（ChEMBL 4,868条，28个新基因）→ 新增 w/o CPI v29 消融变体
  - 蛋白传统特征补充（61基因全覆盖）→ 扩充蛋白特征维度
  - 早停逻辑升级（prot_aupr + prot_auc 联合）
  - 版本号统一为 v28
  - 5种子 × 统计显著性检验（Wilcoxon + Bootstrap CI）
  - 标准化Markdown报告生成

架构:
  - spawn 多进程隔离，每个变体在独立子进程中运行
  - 主进程负责数据加载/拆分/缓存，子进程各自训练
  - 单GPU串行执行，进程隔离确保GPU内存泄漏不累积
  - 每个变体5个随机种子，主进程汇总均值±标准差
"""
...
ABLATION_RESULTS_DIR = L4_RESULTS / "ablation_v28"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(L4_LOGS / "ablation_v28_master.log", encoding="utf-8", mode="w"),
        logging.StreamHandler(sys.stdout),
    ],
    force=True,
)
logger = logging.getLogger("ablation_v28")

# ═══════════════════════════════════════════════════════════
# CPI 泄漏黑名单 — v28 沿用 v25 的黑名单
...
# ═══════════════════════════════════════════════════════════
ABLATION_SEEDS = [42, 123, 456, 789, 1024]
...
ABLATION_VARIANTS = {
    ...
    "Full (v28)": {
        "priority": "P0",
        "desc": "完整v28模型（两阶段+BPR+通路+MLP解码器+课程负采样+疾病节点+表型+CPI v29补充）",
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    # ── P1: 核心组件消融 ──
    "w/o two-stage": {
        "priority": "P1",
        "desc": "取消两阶段预训练，直接全量训练",
        "two_stage": False,    "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "w/o curriculum": {
        "priority": "P1",
        "desc": "移除课程负采样，全部随机负样本",
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": False, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "w/o BPR": {
        "priority": "P1",
        "desc": "移除BPR排序损失，仅Focal Loss",
        "two_stage": True,     "use_bpr": False,      "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "w/o pathway": {
        "priority": "P1",
        "desc": "SAGE和HGT中均禁用通路特征",
        "two_stage": True,     "use_bpr": True,       "use_pathway": False,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "w/o disease": {
        "priority": "P1",
        "desc": "移除疾病节点（GSE61616），退化为三模态",
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": False,  "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "w/o pheno": {
        "priority": "P1",
        "desc": "移除铁死亡表型分类辅助任务",
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": False,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    # ── P2: 补充/对比实验 ──
    "w/o MLP decoder": {
        ...
        "desc": "点积解码器替换MLP解码器",
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": False, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "SAGE only": {
        ...
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": False,
        "skip_cpi_v29": False,
    },
    "HGT only": {
        "priority": "P2",
        ...
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": False,     "use_hgt": True,
        "skip_cpi_v29": False,
    },
    "w/o CPI v29": {
        "priority": "P2",
        "desc": "移除ChEMBL v29补充（4,868条CPI），验证新补充数据的收益",
        "two_stage": True,     "use_bpr": True,       "use_pathway": True,
        "use_mlp_decoder": True, "use_curriculum": True, "use_pheno": True,
        "use_disease": True,   "use_sage": True,      "use_hgt": True,
        "skip_cpi_v29": True,
...
KEY_DATA_FILES = [
    ...
    PROJECT_ROOT / "L1" / "results" / "ppi_network_supplemented.csv",
    ...
    L4_RESULTS / "phenotype_ferroptosis_dataset_v25_clean.csv",
    L4_RESULTS / "disease_gene_edges.csv",
    L4_RESULTS / "cpi_supplement_v29.csv",
    PROJECT_ROOT / "L2" / "results" / "target_protein_features_supplemented.csv",
]
...
_GRAPH_CACHE_NO_V29: dict | None = None  # "w/o CPI v29" 变体用的图缓存
_GRAPH_CACHE_LOCK = mp.Lock()


def build_and_cache_graphs(skip_cpi_v29: bool = False) -> dict:
    """构建图数据并缓存（主进程调用一次）

    Args:
        skip_cpi_v29: 是否跳过 CPI 补充 v28 数据
    """
    global _GRAPH_CACHE, _GRAPH_CACHE_NO_V29

    if skip_cpi_v29 and _GRAPH_CACHE_NO_V29 is not None:
        logger.info("图数据缓存（无v29）命中，跳过重建")
        return _GRAPH_CACHE_NO_V29
    if not skip_cpi_v29 and _GRAPH_CACHE is not None:
    ...
    if skip_cpi_v29:
        # 移除 v29 补充的 CPI 记录（通过基因名和 SMILES 匹配）
        v28_file = L4_RESULTS / "cpi_supplement_v29.csv"
        if v28_file.exists():
            ...
            if "canonical_smiles" in v28_df.columns and "gene" in v28_df.columns:
                ...
                logger.info(f">>> 跳过 CPI v29 补充: 移除 {before - len(cpi_df)} 条记录, 剩余 {len(cpi_df)} 条")
...
def _run_variant_worker(
    ...
    result = {
        ...
        "seed": seed,
        "sage_prot_auc": None, "sage_prot_aupr": None,
        "sage_val_auc": None, "sage_val_aupr": None,
        "hgt_prot_auc": None, "hgt_prot_aupr": None,
        "hgt_val_auc": None, "hgt_val_aupr": None,
    ...
    try:
        ...
        # ── 通路处理 ──
        n_pathways = graphs["n_pathways"] if use_pathway else 0

        gpu_peak = 0.0

        # ── SAGE训练 ──
        if use_sage:
            sub_logger.info(">>> 构建 SAGE 模型")
            sage_model = SAGELinkPredictor(
                ...
                use_mlp_decoder=use_mlp_decoder,
            ).to(DEVICE)
            ...
            sage_model, sage_history = train_sage(
                ...
                use_bpr=use_bpr, use_curriculum=use_curriculum,
                pheno_compound_indices=pheno_idx, pheno_labels=pheno_lab,
                pheno_lambda=PHENO_LAMBDA if use_pheno else 0.0,
            )

            if sage_history:
                result["sage_val_auc"] = max(h.get("auc", 0) for h in sage_history)
                result["sage_val_aupr"] = max(h.get("aupr", 0) for h in sage_history)
                result["sage_prot_auc"] = max(h.get("prot_auc", 0) for h in sage_history)
                result["sage_prot_aupr"] = max(h.get("prot_aupr", 0) for h in sage_history)
                result["sage_best_epoch"] = len(sage_history)
            sub_logger.info(f"  SAGE best: prot_aupr={(result['sage_prot_aupr'] or 0):.4f}, prot_auc={(result['sage_prot_auc'] or 0):.4f}, val_aupr={(result['sage_val_aupr'] or 0):.4f}")

            if torch.cuda.is_available():
                gpu_peak = max(gpu_peak, torch.cuda.max_memory_allocated() / (1024**3))
                torch.cuda.empty_cache()
            del sage_model, sage_history
            gc.collect()

        # ── HGT训练 ──
        if use_hgt:
            sub_logger.info(">>> 构建 HGT 模型")
            hgt_node_feat_dims = {
            ...
            hgt_model = HGTLinkPredictor(
                ...
                dropout=DROPOUT,
                metadata=graphs["hetero_data"].metadata(),
            ...
            if not use_pathway:
                _orig_forward = hgt_model.forward
                hgt_model.forward = lambda x_dict, ei_dict: _orig_forward(
                    x_dict, ei_dict, use_pathway=False)
            ...
            if hgt_history:
                result["hgt_val_auc"] = max(h.get("auc", 0) for h in hgt_history)
                result["hgt_val_aupr"] = max(h.get("aupr", 0) for h in hgt_history)
                result["hgt_prot_auc"] = max(h.get("prot_auc", 0) for h in hgt_history)
                result["hgt_prot_aupr"] = max(h.get("prot_aupr", 0) for h in hgt_history)
                result["hgt_best_epoch"] = len(hgt_history)
            sub_logger.info(f"  HGT best: prot_aupr={result['hgt_prot_aupr']:.4f}")

            if torch.cuda.is_available():
                gpu_peak = max(gpu_peak, torch.cuda.max_memory_allocated() / (1024**3))
                torch.cuda.empty_cache()
            del hgt_model, hgt_history
            gc.collect()
...
def summarize_results(
    ...
    # 打印汇总表
    logger.info("=" * 100)
    logger.info(f"消融实验结果汇总 (v28, {len(ABLATION_SEEDS)} 种子)")
    logger.info("=" * 100)
    ...
    if full_sage is not None:
        logger.info(f"Full (v28) 基线: SAGE prot_aupr={full_sage:.4f}, HGT prot_aupr={full_hgt:.4f}")
...
def run_ablation(
    ...
    logger.info("=" * 60)
    logger.info("消融实验运行器 (Ablation Study v28)")
    logger.info("=" * 60)
    ...
    PROTEIN_VAL_SPLIT = 0.50    # v28: 50%蛋白验证比例以提升CPI蛋白验证统计效力
    ...
    for v_idx, (name, cfg) in enumerate(variants.items()):
        # ── "w/o CPI v29" 需要单独的图和数据拆分 ──
        use_no_v28 = cfg.get("skip_cpi_v29", False)
        if use_no_v28:
            logger.info(">>> 构建无v29图数据缓存")
            graphs_no_v28 = build_and_cache_graphs(skip_cpi_v29=True)
            # 重新计算拆分
            ...
            # 移除 v29 记录
            v28_file_path = L4_RESULTS / "cpi_supplement_v29.csv"
            ...
            logger.info(f"无v29冷启动拆分: {len(train_compounds_v)} train / {len(val_compounds_v)} val 化合物")
            logger.info(f"无v29蛋白冷启动: {len(train_proteins_v)} train / {len(val_proteins_v)} val 蛋白")
        else:
        ...
        for seed_idx, seed in enumerate(ABLATION_SEEDS):
            ...
            # 保存中间结果
            inter_path = ABLATION_RESULTS_DIR / "ablation_v28_intermediate.json"
            try:
                serializable = []
                for r in all_results:
                    sr = {}
    ...
    # ── 结果汇总 ──
    summary_path = ABLATION_RESULTS_DIR / "ablation_v28_results.csv"
    df = summarize_results(all_results, output_path=summary_path)

    # 保存完整 JSON
    json_path = ABLATION_RESULTS_DIR / "ablation_v28_full_results.json"
    try:
        serializable = []
        for r in all_results:
            sr = {}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="消融实验运行器 v28")
    ...
    if args.dry_run:
        ...
        for name, cfg in ABLATION_VARIANTS.items():
            ...
            logger.info(f"    two_stage={cfg.get('two_stage')}, "
