...
def train_sage(
    ...
    # v27: 预计算扁平 (化合物, 蛋白) 正样本对张量
    if precomputed_pos:
        _pos_pair_comp_list = []
        _pos_pair_prot_list = []
        for _comp, _prots in precomputed_pos.items():
            for _prot in _prots:
                _pos_pair_comp_list.append(_comp)
                _pos_pair_prot_list.append(_prot)
        pos_pair_comp_all = torch.tensor(_pos_pair_comp_list, dtype=torch.long)
        pos_pair_prot_all = torch.tensor(_pos_pair_prot_list, dtype=torch.long)
    else:
        pos_pair_comp_all = torch.tensor([], dtype=torch.long)
        pos_pair_prot_all = torch.tensor([], dtype=torch.long)
    ...
    def _train_one_epoch(
        ...
        for batch_start in range(0, len(shuffled_compounds), batch_size):
            ...
            # v27: 向量化正样本对构建
            if len(pos_pair_comp_all) == 0:
                continue
            _max_global = max(node_to_local.keys(), default=0)
            if len(pos_pair_comp_all) > 0:
                _max_global = max(_max_global, int(pos_pair_comp_all.max().item()),
                                  int(pos_pair_prot_all.max().item()))
            nl_t = torch.full((_max_global + 1,), -1, dtype=torch.long)
            for g, l in node_to_local.items():
                nl_t[g] = l
            _max_local = max(node_to_local.values(), default=0)
            ltp_t = torch.full((_max_local + 1,), -1, dtype=torch.long)
            for lpos, ppos in local_to_prot_pos.items():
                ltp_t[lpos] = ppos
            l2c_t = torch.full((_max_local + 1,), -1, dtype=torch.long)
            for bi, s in enumerate(batch_seeds):
                ci = batch_idx_to_comp_idx.get(bi)
                if ci is not None and s in node_to_local:
                    l2c_t[node_to_local[s]] = ci
            batch_seeds_t = torch.tensor(batch_seeds, dtype=torch.long)
            pair_mask = torch.isin(pos_pair_comp_all, batch_seeds_t)
            pair_comp = pos_pair_comp_all[pair_mask]
            pair_prot = pos_pair_prot_all[pair_mask]
            lc = nl_t[pair_comp]
            lp = nl_t[pair_prot]
            valid = (lc >= 0) & (lp >= 0)
            lc, lp = lc[valid], lp[valid]
            pp = ltp_t[lp]
            ci = l2c_t[lc]
            valid_final = (pp >= 0) & (pp < n_batch_prots) & (ci >= 0)
            pos_src_t = ci[valid_final].to(device)
            pos_dst_t = pp[valid_final].to(device)
            if len(pos_src_t) == 0:
                continue
