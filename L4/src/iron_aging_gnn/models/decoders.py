...
v32: 新增低秩双线性解码器 (Low-Rank Bilinear Decoder)，
     作为 MLP 解码器和点积解码器的替代方案。
v33: 新增 ResidueAwareBilinearDecoder，真正利用蛋白逐残基 ESM-2 特征
     进行化合物-残基级双线性注意力交互。
...
class ResidueAwareBilinearDecoder(nn.Module):
    ...
    通过化合物嵌入作为 query，对蛋白残基做注意力加权，实现真正的
    化合物-残基层面交互，而非全局向量点积。

    v33: 基于 GraphBAN 双线性注意力思想，适配本项目的 packed 残基存储格式。
    ...
    def __init__(self, comp_dim: int, residue_dim: int = 640, rank: int = 64,
        ...
        # 这些特征由外部通过 register_residue_buffers 注册；
        ...
        # 图蛋白索引 -> residue 文件索引（或 unknown placeholder 索引）
    ...
    def register_residue_buffers(self, embeddings: torch.Tensor, offsets: torch.Tensor,
        ...
        Args:
            ...
            prot_to_residue_idx: (n_graph_proteins,) 图蛋白索引 -> residue 文件索引，
                                 缺失蛋白指向 unknown placeholder
            residue_device: 残基张量驻留设备，默认 "cpu" 防止 8GB GPU OOM
        """
        self.max_len = max_len
