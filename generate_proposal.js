...
children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 300 },
  children: [new TextRun({ text: "广西道地壮药桂艾活性成分β-石竹烯靶向Nrf2/ACSL4通路", size: 28, bold: true, font: "黑体" })] }));
children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 600 },
  children: [new TextRun({ text: "抑制铁衰老改善脑缺血再灌注损伤的机制研究", size: 28, bold: true, font: "黑体" })] }));
...
children.push(para([
  { text: "① 铁衰老（ferro-aging）——铁死亡驱动细胞衰老的新型病理轴", bold: true }
]));

children.push(para("铁死亡与细胞衰老并非两个孤立的病理过程。近年研究揭示，二者之间存在密切的交互作用，共同构成一个\"铁死亡驱动细胞衰老\"的新型病理轴——即\"铁衰老\"（ferro-aging）。2026年，Liu等[5]在Cell Metabolism发表的灵长类动物研究首次系统定义了ferro-aging这一概念：铁过载通过ACSL4介导的脂质过氧化，驱动细胞进入衰老状态，形成一个从铁代谢紊乱到脂质过氧化再到细胞衰老的完整级联反应。"));
...
children.push(para([
  { text: "4-HNE-p53羰基化——连接铁死亡与衰老的关键分子开关：", bold: true },
  "在铁死亡过程中，脂质过氧化产生的活性醛类（尤其是4-羟基壬烯醛，4-HNE）可与蛋白质的半胱氨酸、组氨酸、赖氨酸残基发生共价结合，形成蛋白质羰基化修饰。近年研究发现，4-HNE可直接羰基化修饰p53蛋白的特定赖氨酸残基，增强p53的稳定性和转录活性[6]。活化的p53一方面通过上调p21诱导细胞周期停滞，另一方面可转录抑制SLC7A11（System Xc⁻的关键亚基），进一步削弱细胞的抗氧化防御能力，形成\"脂质过氧化→p53羰基化→SLC7A11↓→更多脂质过氧化\"的正反馈环路。这一4-HNE-p53羰基化-SLC7A11分子开关，可能是铁死亡驱动SIPS的核心机制。"
]));
...
children.push(bullet("验证BCP对Keap1-Nrf2复合物的调控：分子对接预测BCP与Keap1疏水口袋的结合模式；细胞热位移分析（CETSA）验证BCP与Keap1的直接结合。"));

children.push(para([
  { text: "4. BCP对4-HNE-p53羰基化-SLC7A11分子开关的调控", bold: true }
]));
children.push(bullet("检测BCP处理后，细胞内4-HNE含量、p53羰基化水平、p53核转位、SLC7A11表达的变化。"));
children.push(bullet("在p53敲低/过表达细胞中，观察BCP的抗SIPS效应是否依赖p53。"));
children.push(bullet("验证\"BCP→Nrf2→GPX4↑→4-HNE↓→p53羰基化↓→SLC7A11↑→进一步抑制铁死亡\"的正反馈解除机制。"));
...
children.push(bullet([
  { text: "实验分组：", bold: true }, "假手术组、模型组、BCP低剂量组（102 mg/kg）、BCP中剂量组（204 mg/kg）、BCP高剂量组（408 mg/kg）、Liproxstatin-1阳性组（10 mg/kg）、BCP+ML385组（30 mg/kg），每组12只。"
]));
children.push(bullet([
...
children.push(heading3("研究内容四：桂艾挥发油的化学成分分析及药效增益初步探索"));
...
children.push(para([
  { text: "2. 桂艾挥发油与BCP单体的药效初步比较", bold: true }
]));
...
const techRows = [
  [
    { text: "上 层\n现 象 验 证", fill: "E8F5E9" },
    { text: "CIRI半暗带铁依赖性SIPS的存在性验证", fill: "E8F5E9" }
  ],
  [
    { text: "", fill: "FFFFFF" },
  ...
  [
    { text: "中 层\n机 制 解 析", fill: "E3F2FD" },
    { text: "4-HNE-p53羰基化-SLC7A11分子开关 + BCP调控机制", fill: "E3F2FD" }
  ],
  [
    { text: "", fill: "FFFFFF" },
  ...
  [
    { text: "", fill: "FFFFFF" },
    { text: "4-HNE→p53羰基化→SLC7A11↓\nIP/IB | ChIP | 报告基因 | 羰基化抑制剂\n\n            BCP→Nrf2→GPX4↑/ACSL4↓→4-HNE↓\n            Nrf2抑制剂/敲低 | CETSA | 分子对接", fill: "FFFFFF" }
  ],
  [
    { text: "", fill: "FFFFFF" },
  ...
  [
    { text: "", fill: "FFFFFF" },
    { text: "↓ 整体药效 ↓            ↓ Nrf2依赖 ↓            ↓ 桂艾比较 ↓", fill: "FFFFFF" }
  ],
  [
    { text: "", fill: "FFFFFF" },
    { text: "MCAO/R模型\nTTC | mNSS | 转棒 | 水迷宫\nSA-β-gal | p53羰基化 | SASP\n\nNrf2⁻/⁻小鼠\n验证Nrf2依赖性\n\n            桂艾挥发油 vs BCP单体            \n            头对头药效初步比较", fill: "FFFFFF" }
  ],
  [
    { text: "", fill: "FFFFFF" },
    { text: "↓\n最终结论：BCP通过Nrf2/ACSL4通路抑制铁依赖性SIPS，改善CIRI远期预后", fill: "F3E5F5" }
  ],
];
...
children.push(bullet("构建AAV-shACSL4载体，通过脑立体定位注射验证ACSL4在铁依赖性SIPS中的核心驱动作用。"));
...
const refs = [
  ...
  "[6] Liu Z, Ye X, Li X, et al. 4-HNE mediates p53 carbonylation and activation in iron overload-induced cellular senescence. Redox Biology, 2024, 77: 103187.",
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("标书_修订版_广西道地壮药桂艾BCP靶向Nrf2通路抑制铁衰老改善CIRI.docx", buffer);
  console.log("标书Word文档生成成功！");
  console.log("文件路径：d:\\铁衰老 绝不重蹈覆辙\\标书_修订版_广西道地壮药桂艾BCP靶向Nrf2通路抑制铁衰老改善CIRI.docx");
});
