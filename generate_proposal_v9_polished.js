...
children.push(p('综上所述，脑缺血再灌注损伤是急性缺血性脑卒中治疗中的重大临床难题，其病理机制涉及急性期多种调节性细胞死亡和慢性期神经炎症与修复障碍。铁死亡作为急性期神经元死亡的关键形式已得到广泛认可；细胞衰老则被认为是缺血半暗带慢性化的重要推手。然而，二者之间的因果联系\u2014\u2014即铁死亡是否以及如何驱动缺血半暗带中的细胞衰老（铁依赖性SIPS）\u2014\u2014目前尚不清楚。特别是，4-HNE作为脂质过氧化的核心产物，其作用的机制选择性（优先激活Keap1-Nrf2防御还是转向p53介导的衰老）是决定亚致死铁死亡压力最终细胞命运的关键科学问题。'));
children.push(p('基于文献线索和我们前期的网络药理学研究，我们推测：CIRI半暗带中存在亚致死性铁死亡压力驱动的铁依赖性SIPS；4-HNE作用存在时间依赖性的机制转换\u2014\u2014早期激活Keap1-Nrf2防御，晚期超过阈值后通过DDR-p53通路驱动衰老；4-HNE-p53-SLC7A11正反馈环路是铁依赖性SIPS维持和放大的核心分子机制；Nrf2作为铁死亡防御与衰老抑制的共同枢纽，是理想的干预靶点；广西道地壮药桂艾的活性成分\u03b2-石竹烯可通过激活Nrf2通路，同时阻断铁死亡急性期损伤和铁依赖性SIPS慢性损害，从而改善CIRI远期预后。'));
children.push(p('本研究的理论意义在于：提出并验证缺血诱导的铁依赖性SIPS这一新型病理概念，丰富对CIRI慢性化机制的认识；阐明4-HNE作用的机制选择性及其在铁依赖性SIPS启动中的开关作用，为铁死亡驱动衰老的分子机制提供新的视角。实践意义在于：阐明壮瑶药桂艾活性成分\u03b2-石竹烯的神经保护新机制，为开发以铁衰老为靶点的脑卒中治疗药物提供理论依据和先导化合物，也为民族医药现代化研究提供可借鉴的研究范式。'));
...
children.push(p('本研究中铁衰老基因集的构建遵循\u201c多源整合、严格筛选、功能验证\u201d的原则，具体流程如下：首先，从FerrDb数据库获取经文献curation的铁死亡相关基因（驱动基因和抑制基因），从CellAge数据库获取细胞衰老相关基因（Senescence-Associated Genes, SAGs），从Gene Ontology数据库获取\u201ccellular response to oxidative stress\u201d\u201ciron ion homeostasis\u201d\u201clipid peroxidation\u201d等功能条目下的基因。其次，取铁死亡基因集与细胞衰老基因集的交集，获得\u201c双身份基因\u201d；进一步通过文本挖掘PubMed文献中同时提及\u201cferroptosis\u201d和\u201csenescence\u201d的基因，补充入基因集。第三，通过STRING v12.0蛋白质相互作用网络分析，保留在网络中具有至少3个连接的基因，去除孤立节点，确保基因集的功能连通性。最终获得96个铁衰老相关基因，涵盖铁代谢调控、脂质过氧化与抗氧化防御、DNA损伤与衰老调控以及SASP相关因子四大功能类别。基因集的完整列表见附录1。'));

children.push(pNoIndent([
  new TextRun({ text: '（2）四数据集铁衰老评分的疾病关联性', size: 24, font: '宋体', bold: true })
]));
children.push(p('本研究整合了4个脑缺血时间进程数据集\u2014\u2014GSE104036（小鼠MCAO，RNA-seq，27样本，0\u201472h）、GSE16561（人缺血性脑卒中，Illumina微阵列，63样本）、GSE61616（大鼠MCAO，Affymetrix微阵列，15样本）及GSE97537（大鼠MCAO，Affymetrix微阵列，12样本）。采用单样本基因集富集分析（ssGSEA），基于Barbie等（Nature Protocols, 2009）[9]的秩加权富集统计算法，计算每个样本的铁死亡、细胞衰老及铁衰老（96基因集）评分。结果显示，在全部4个数据集中，铁衰老评分的疾病\u2014对照效应量（Cohen\u2019s d）均大于铁死亡评分和衰老评分，表明铁衰老基因集捕获的转录信号与缺血性脑损伤的关联最为紧密，在跨物种、跨平台中具有稳健性。'));
...
children.push(p('时序分析显示，铁死亡与铁衰老评分均在再灌注后6小时达到峰值，随后下降；而经典细胞衰老评分并未在后期时间点升高，反而在急性期至亚急性期呈下降趋势。这一发现不支持经典的\u201c铁死亡上升\u2192衰老上升\u2192铁衰老过渡\u201d序列模式。据此，本项目明确指出：当前基于转录组的铁衰老评分主要捕获的是铁死亡相关转录状态，而非经典意义上的衰老程序\u2014\u2014评分中\u201c衰老\u201d成分的贡献主要来自p53/p21通路的早期激活（DNA损伤应答的即时反应），而非完全建立的衰老表型。因此，本项目将后续实验验证的核心定位为\u201c铁死亡压力是否驱动细胞进入衰老状态\u201d这一因果关系的证明，而非将转录组评分作为铁衰老存在的直接证据。'));
children.push(p('以GSE104036中位数划分的高铁衰老活性组为因变量，以铁衰老基因集表达矩阵为预测变量，采用L1正则化逻辑回归进行特征选择。为确保稳定性，重复50次随机子采样的6折交叉验证，保留选择频率大于50%的基因。结果筛选出5个稳定的CIRI-铁衰老特征基因：SAT1（96%）、EBF3（88%）、KLF6（88%）、LIFR（72%）及CD74（70%）。内部交叉验证AUC为0.73 \u00b1 0.09，置换检验P = 0.002。将训练好的模型应用于3个独立数据集，预测概率与独立计算的铁衰老评分呈显著正相关：GSE16561（\u03c1 = 0.56，P < 0.0001）、GSE61616（\u03c1 = 0.75，P < 0.0001）、GSE97537（\u03c1 = 0.88，P < 0.0001），有力支持了五基因特征对铁死亡相关转录活性的预测效能。'));

children.push(h3('2. 石竹烯靶点与铁衰老调控网络的拓扑收敛'));
children.push(p('采用交集+网络邻近扩展双路径策略确定石竹烯干预脑缺血铁衰老的最终核心基因集：最终核心基因集 = （CIRI-铁衰老候选基因 \u2229 石竹烯高置信度靶点） \u222a 网络邻近扩展的铁死亡关键基因。Part A通过五基因CIRI-铁衰老签名与石竹烯427个高置信度靶点（SwissTargetPrediction + STITCH联合预测）取交集，仅包含SAT1（选择频率96%），提示SAT1是石竹烯直接作用于铁衰老调控的核心靶点。Part B通过一阶网络邻近分析，在STRING v12.0（人，combined score > 700）[10]网络中，石竹烯靶点的直接邻居中识别出337个铁死亡驱动基因邻近基因（44个直接靶点 + 293个一阶邻居）。超几何检验显示该富集具有极高度统计学意义（P = 2.48 \u00d7 10\u207b\u2074\u00b3），表明石竹烯靶点群与铁死亡调控网络存在高度拓扑关联。刘胜伟等[19]的网络药理学研究同样证实BCP作用于CIRI的靶点富集于p53、MAPK、NF-\u03baB等通路，关键靶点包括IL-6、TNF、TP53等，与本项目的分析结果高度一致。'));

children.push(h3('3. 免疫浸润与铁衰老的协同激活及WGCNA验证'));
children.push(p('基于特征基因集的ssGSEA免疫浸润分析显示，在GSE104036的27个样本中，铁衰老评分与多种免疫细胞丰度存在显著相关。其中，中性粒细胞（r = 0.651，P = 2.3 \u00d7 10\u207b\u2074）和M2型巨噬细胞（r = 0.613，P = 6.7 \u00d7 10\u207b\u2074）与铁衰老评分呈强正相关；而小胶质细胞稳态标志（r = -0.738，P = 1.1 \u00d7 10\u207b\u2075）和星形胶质细胞（r = -0.567，P = 0.002）呈显著负相关。需要说明的是，ssGSEA方法在区分M2型巨噬细胞与小胶质细胞亚型时存在一定局限性\u2014\u2014CD206等标志物在某些活化状态的小胶质细胞中也有表达，可能导致算法对细胞类型的判定存在交叉。为弥补这一缺陷，后续实验验证阶段将采用多色免疫荧光进行细胞类型的精确鉴定。在样本量最大的人脑缺血数据集GSE16561（63样本）中，加权基因共表达网络（WGCNA）验证显示，337个核心基因中有97个被分配至有意义的共表达模块，证实核心基因集在人脑缺血的共表达调控网络中处于核心位置。'));
...
children.push(p('本部分为项目研究重心，旨在明确CIRI后半暗带中是否存在铁依赖性SIPS，及其时空分布特征、细胞类型和核心分子机制，重点解答4-HNE作用的机制选择性问题。采用C57BL/6J小鼠MCAO/R模型（缺血60 min），在再灌注后6 h、24 h、3 d、7 d、28 d五个时间点取材。运用免疫荧光多重标记技术（NeuN/GFAP/Iba-1分别与铁死亡标志物GPX4/4-HNE/FTH1及衰老标志物p21/\u03b3H2AX共定位）、透射电镜、铁含量比色法及普鲁士蓝染色，系统定量分析铁衰老双阳性细胞的时空分布规律及细胞类型。'));
...
children.push(p('本部分在整体动物水平验证BCP对CIRI的神经保护效应，重点关注远期功能预后，确认Nrf2依赖性。采用C57BL/6J小鼠MCAO/R模型，设假手术组、模型组、BCP低/中/高剂量组、Liproxstatin-1阳性药组、BCP+ML385组，每组12只。再灌注后即刻首次给药，给药14天，28天进行行为学终点评价。'));

children.push(pNoIndent([
  new TextRun({ text: '给药方案与Nrf2验证策略：', size: 24, font: '宋体', bold: true }),
  new TextRun({ text: '预实验设5个剂量梯度（50\u2014600 mg/kg）确定治疗模式ED50和最大有效剂量；通过3个时间点GC-MS测定脑组织BCP浓度，确定给药频率。采用\u201c药理学抑制+脑区特异性敲降\u201d双重验证Nrf2依赖性：ML385（30 mg/kg）药理学抑制为主，AAV-shNrf2脑立体定位局部敲降为辅，二者互为补充。短期评价（1\u20143 d）：TTC染色、脑水肿、mNSS评分；远期评价（28 d）：足误实验、转棒实验。28 d脑组织行SA-\u03b2-gal染色、铁死亡-衰老双阳性细胞定量及SASP标志物检测，关联BCP药效与铁依赖性SIPS抑制。', size: 24, font: '宋体' })
]));

children.push(h2('研究内容四：桂艾挥发油与BCP单体的药效比较研究'));
children.push(pNoIndent([
  new TextRun({ text: '研究内容：', size: 24, font: '宋体', bold: true }),
  new TextRun({ text: '（1）桂艾挥发油制备与质量控制：水蒸气蒸馏法提取广西道地桂艾挥发油，GC-MS成分分析，确保BCP含量15%\u201435%。（2）药效比较：在MCAO/R模型中比较桂艾挥发油组（独立剂量梯度）与等BCP摩尔含量的BCP单体组在梗死体积、铁死亡和衰老标志物方面的差异，初步判断多成分协同效应。（3）机制初步探索：比较两组Nrf2通路激活强度差异。', size: 24, font: '宋体' })
]));
...
children.push(h2('5.1 实验材料与方法'));
children.push(bullet('实验动物：SPF级C57BL/6J小鼠（22-25 g，雄性），购自北京维通利华；屏障环境饲养，12 h光暗循环。'));
children.push(bullet('药物与试剂：\u03b2-石竹烯（Sigma-Aldrich，\u226598.5%）、桂艾挥发油（自制，GC-MS鉴定）、Erastin/RSL3/Liproxstatin-1（MCE）、DFO、ML385、4-HNE（Sigma）、SA-\u03b2-gal染色试剂盒（CST）、C11-BODIPY 581/591（Invitrogen）。'));
children.push(bullet('主要抗体与病毒：抗-GPX4、ACSL4、FTH1、p53、p-p53、p21、\u03b3H2AX、4-HNE、Nrf2、Keap1、HO-1、SLC7A11、NeuN、GFAP、Iba-1等（CST/Abcam/Proteintech）；AAV-shACSL4、AAV-ACSL4、AAV-shNrf2、AAV-p53 WT及突变体（汉恒/吉凯，滴度\u22651\u00d710\u00b9\u00b2 vg/mL）。'));
children.push(bullet('道地药材：广西道地桂艾采自广西药用植物园GAP基地，经广西中医药大学鉴定。'));
children.push(bullet('模型与检测：小鼠MCAO/R（改良线栓法，缺血60 min）；原代皮层神经元/星形胶质细胞培养；OGD/R模型；铁死亡检测（Fe\u00b2\u207a探针、C11-BODIPY、MDA、GSH、透射电镜）；衰老检测（SA-\u03b2-gal、p21/p16、SASP、免疫荧光）；分子生物学（Western Blot、qRT-PCR、IP、ChIP-qPCR、分子对接、CETSA）；行为学（转棒、足误）；脑立体定位注射AAV。'));

children.push(h2('5.2 统计学分析'));
children.push(p('数据采用GraphPad Prism 9.0和SPSS 26.0分析。计量资料以均数\u00b1标准差表示。两组比较采用t检验或Mann-Whitney U检验；多组比较采用单因素方差分析，两两比较用Tukey或Dunnett法；重复测量数据采用重复测量方差分析。高通量检测采用Benjamini-Hochberg法FDR校正（q < 0.05）。以梗死体积为主要终点，按Cohen d \u2248 1.2、\u03b1 = 0.05、Power = 0.80估算，每组n = 12（含20%脱落率）。P < 0.05（或q < 0.05）为差异有统计学意义。'));
...
children.push(h2('7.1 理论与技术可行性'));
children.push(p('铁死亡和细胞衰老是当前CIRI研究的两个热点领域，二者交互作用（铁衰老）是前沿交叉方向，具有坚实理论基础。铁死亡在CIRI中的作用已被众多实验室独立验证[3,7,13]；细胞衰老参与缺血后脑损伤慢性化也得到越来越多证据支持[8]；4-HNE作为脂质过氧化产物诱导衰老的效应已在多种细胞类型中得到证实[2]；Nrf2作为二者共同防御枢纽的地位已经确立[12]。本项目组前期通过4个脑缺血数据集整合分析，证实了铁衰老转录特征在脑缺血中的稳健存在：铁衰老评分在4个数据集中均表现出最大的疾病-对照效应量，五基因CIRI-铁衰老签名在3个独立数据集中得到验证，1000次置换检验证实基因集具有高度特异性（P = 0.001）。网络药理学分析预测了BCP靶点与铁死亡调控网络的高度拓扑关联（超几何检验P = 2.48 × 10⁻⁴³）。'));

children.push(p('实验材料与技术均成熟可靠。广西道地桂艾来源明确，BCP标准品为商品化试剂，AAV载体和Nrf2相关工具鼠均已商用可购。项目涉及的小鼠MCAO/R模型、原代神经细胞培养、Western Blot、qRT-PCR、免疫荧光共定位等均为成熟常规技术。铁死亡和衰老的特异性检测方法已有大量文献支持，试剂盒商品化程度高。行为学检测均为神经科学研究的标准范式。'));

children.push(h2('7.2 前期工作基础'));
children.push(p('本团队前期已在脑缺血、铁死亡和天然药物神经保护领域积累了扎实研究基础。在计算生物学层面，已完成4个脑缺血数据集的整合分析，建立了CIRI-铁衰老五基因签名，验证了BCP靶点与铁死亡调控网络的高度拓扑富集，构建了核心PPI子网，完成了免疫浸润分析及WGCNA共表达网络验证。在实验层面，项目组已建立小鼠MCAO/R模型和原代神经细胞培养体系。Hu等[3]已证实BCP通过Nrf2/HO-1通路减轻大鼠脑缺血再灌注损伤，为本项目延伸研究提供了直接前期支持。项目组与广西中医药大学中药鉴定教研室、广西药用植物园等单位建立了良好合作关系，可为桂艾药材来源鉴定和成分分析提供技术支持。'));

children.push(h1('八、特色与创新之处'));

children.push(h2('8.1 理论创新：提出CIRI中铁依赖性SIPS新假说及4-HNE机制选择性新视角'));
children.push(p('现有研究多将铁死亡和细胞衰老视为CIRI中两个独立过程，分别关注急性期和慢性期，对二者因果联系缺乏系统探讨。铁死亡驱动衰老的概念主要在肿瘤和自然衰老领域提出，在缺血性脑损伤中研究几近空白。特别是4-HNE作用的机制选择性\u2014\u2014为何在某些情况下激活Nrf2防御、另一些情况下驱动p53衰老\u2014\u2014这一关键问题尚未解答。本项目率先提出CIRI半暗带中存在缺血诱导的铁依赖性SIPS假说，并进一步提出4-HNE作用存在时间依赖性机制转换的新视角：早期通过Keap1修饰激活Nrf2防御，晚期超过阈值后通过DDR-p53通路转向衰老。这一假说将CIRI急性期损伤与慢性期恶化机制贯通，为理解缺血性脑损伤慢性化提供新视角，也为寻找同时干预急慢性阶段的治疗靶点开辟新思路。'));

children.push(h2('8.2 机制创新：揭示BCP通过Nrf2双阻断铁死亡-衰老的新机制'));
children.push(p('BCP的神经保护作用已有报道，但多停留在宏观药效层面，对其细胞分子机制解析不够深入；现有研究多关注单一靶点或单一病理过程，缺乏对信号调控网络的系统认识；BCP对铁死亡和衰老的研究多为独立报道，尚未将二者联系起来。本项目系统解析BCP通过激活Nrf2通路协同抑制铁死亡与铁依赖性SIPS的分子机制，明确BCP\u2192Nrf2\u2192GPX4\u2191\u21924-HNE\u2193\u2192阻断p53衰老转向\u2192SLC7A11\u2191完整信号轴，验证BCP对4-HNE-p53-SLC7A11正反馈环路的调控。首次阐明壮瑶药桂艾活性成分BCP通龙路火路、除毒邪功效的铁衰老干预科学内涵，为天然小分子同时靶向急性死亡和慢性衰老提供新的作用范式。'));

children.push(h2('8.3 模式创新：构建壮瑶药-铁衰老的整合研究新模式'));
children.push(p('民族药现代化研究常停留在成分鉴定和活性筛选层面，与前沿生物学问题结合不够紧密；传统网络药理学研究多为描述性分析，与后续实验验证衔接不够紧密。本项目构建基于壮瑶医药理论的道地药材-功效-核心成分-铁衰老靶点-信号通路精准整合研究模式，将壮瑶医\u201c通龙路火路、除毒邪\u201d传统功效与铁死亡-衰老交互的现代前沿生物学有机结合。明确BCP单体与桂艾挥发油分别代表\u201c精准机制\u201d与\u201c整体观\u201d两个研究层面，避免单体研究脱离民族药整体特色的常见问题。前期网药分析的系统布局为后续实验提供了明确靶点和通路方向，体现了从计算预测到实验验证的转化医学研究思路，为民族药现代化研究提供可复制的范例。'));

children.push(h1('九、年度研究计划及预期研究成果'));

children.push(h2('9.1 第一年（2027年）'));
children.push(bullet('建立小鼠MCAO/R模型并优化方法，确保稳定性和重复性。'));
children.push(bullet('完成铁依赖性SIPS的时空定位研究（5个时间点、3个脑区、3种细胞类型）。'));
children.push(bullet('完成4-HNE机制选择性时序验证（Keap1修饰vs p53修饰动力学）。'));
children.push(bullet('完成p53关键修饰位点鉴定及ACSL4双向验证的病毒包装与注射。'));
children.push(bullet('完成桂艾挥发油提取与GC-MS成分分析。'));
children.push(bullet('预期成果：发表SCI论文1篇，申请发明专利1项，培养硕士研究生1名。'));

children.push(h2('9.2 第二年（2028年）'));
children.push(bullet('建立稳定的体外铁依赖性SIPS细胞模型并完成铁依赖性严格验证。'));
children.push(bullet('完成BCP抗铁依赖性SIPS药效评价及Nrf2依赖性细胞水平验证。'));
children.push(bullet('完成BCP对4-HNE-p53-SLC7A11分子轴调控的系统检测。'));
children.push(bullet('完成C11-BODIPY+SA-β-gal双重染色共定位验证及分子对接/CETSA探索。'));
children.push(bullet('预期成果：发表SCI论文1-2篇，培养硕士研究生1名。'));

children.push(h2('9.3 第三年（2029年）'));
children.push(bullet('完成BCP给药剂量预实验及整体药效评价（短期指标+远期行为学）。'));
children.push(bullet('完成Nrf2依赖性双重验证（ML385 + AAV-shNrf2）。'));
children.push(bullet('完成脑组织铁依赖性SIPS标志物检测及桂艾挥发油与BCP药效比较。'));
children.push(bullet('数据整理、统计分析、论文撰写与项目结题验收。'));
