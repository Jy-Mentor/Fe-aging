...
generate_fig1a <- function() {
  ...
  ggplot(tab1, aes(x = axis_label, y = match_pct, fill = Evidence)) +
...
generate_fig1b <- function() {
  ...
  ggplot(kegg_top, aes(x = label, y = cross_omics_score)) +
...
generate_fig1c <- function() {
  ...
  ggplot(sat1_plot, aes(x = label, y = log2FC_aging, fill = match_label)) +
    geom_col(width = 0.7, color = "black", linewidth = 0.4) +
...
# 仅在直接运行 (Rscript 19_methodology_figures_v2.R) 时才执行, source() 时不执行
