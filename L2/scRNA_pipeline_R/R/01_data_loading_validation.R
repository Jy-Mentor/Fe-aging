...
step01_validate_seurat <- function(cfg) {
  ...
  cond_summary <- paste(names(cond_tab), cond_tab, sep = "=", collapse = ", ")
  log_info("[Step1] Conditions: ", cond_summary)
  ...
  if (inherits(expr_data, c("dgCMatrix", "dgRMatrix", "dgTMatrix", "CsparseMatrix"))) {
    ...
    log_info("[Step1] Sparse matrix - sampled ", length(sample_idx),
             " non-zero entries, missing frac: ",
             sprintf("%.4f%%", missing_frac * 100))
  } else {
    missing_frac <- mean(is.na(expr_data))
    log_info("[Step1] Missing value fraction: ",
             sprintf("%.4f%%", missing_frac * 100))
  }
  ...
  log_info("[Step1] nFeature_RNA: min=", min(nfeat),
           ", median=", median(nfeat), ", max=", max(nfeat))

  # 1.9 percent.mt
  if ("percent.mt" %in% meta_cols) {
    pmt <- seu@meta.data$percent.mt
    log_info("[Step1] percent.mt: min=", round(min(pmt), 3),
             ", median=", round(median(pmt), 3),
             ", max=", round(max(pmt), 3))
  }
