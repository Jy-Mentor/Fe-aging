...
_MYGENE_ORTHOLOG_CACHE = None


def _load_mygene_ortholog_cache():
    """Load rat -> human ortholog cache generated by mygene.info / HomoloGene."""
    global _MYGENE_ORTHOLOG_CACHE
    if _MYGENE_ORTHOLOG_CACHE is not None:
        return _MYGENE_ORTHOLOG_CACHE

    cache_file = os.path.join(RESULT_DIR, "rat_to_human_ortholog_mygene.csv")
    if not os.path.exists(cache_file):
        logger.warning("MyGene ortholog cache not found: %s", cache_file)
        _MYGENE_ORTHOLOG_CACHE = {}
        return _MYGENE_ORTHOLOG_CACHE

    try:
        df = pd.read_csv(cache_file)
        cache = dict(zip(df["rat_symbol"], df["human_symbol"]))
        logger.info(
            "Loaded mygene ortholog cache: %d rat -> human mappings from %s",
            len(cache), cache_file
        )
        _MYGENE_ORTHOLOG_CACHE = cache
        return _MYGENE_ORTHOLOG_CACHE
    except Exception as e:
        logger.warning("Failed to load mygene ortholog cache: %s", e)
        _MYGENE_ORTHOLOG_CACHE = {}
        return _MYGENE_ORTHOLOG_CACHE
...
def get_species_ortholog_map():
    ...
    Loads the mygene.info / HomoloGene rat → human cache (generated by
    L4/scripts/build_rat_human_ortholog_mygene.py) and merges it with a
    curated mapping. Curated entries override the cache for known
    divergent orthologs. Symbols not found in either source fall back
    ...
    mygene_cache = _load_mygene_ortholog_cache()
    merged = {**mygene_cache, **curated}
    logger.info(
        "Species ortholog map: %d curated + %d mygene cache = %d total mappings",
        len(curated), len(mygene_cache), len(merged)
    )
    return merged
...
def map_to_human_ortholog(gene_symbol, species, ortholog_map):
    ...
    Priority:
      1. Human symbols are returned uppercased as-is.
      2. Curated / mygene.info ortholog map (curated takes precedence).
      3. Uppercase conversion fallback for symbols that are conserved.
    """
    if species == "Human":
        return gene_symbol.upper()

    # Use merged ortholog map (mygene cache + curated overrides)
    if gene_symbol in ortholog_map:
        return ortholog_map[gene_symbol]

    # Fallback: uppercase conversion for conserved symbols
